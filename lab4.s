...
mul $t0, $a2, $a2  	# number of bytes (dim * dim)


loop:
lbu $t5, 0($a0)		# read one byte from input buffer
addi $a0, $a0, 1	# point to next byte
bge $t5, $a3, whiteify	# over the threshold
sb $zero, 0($a1)	# make it black
j donefilter
whiteify:
li $t6, 0xFF		# make it white
sb $t6, 0($a1)

donefilter:
addi $a1, $a1, 1	# point to next byte output buffer
addi $t0, $t0, -1	# one more byte read
bnez $t0, loop		# not done yet
...
move $t0, $zero   	# Current X
move $t1, $zero   	# Current Y
mul $t5, $a3, $a3 	# Loop Counter (total bytes)

mainLoop:
# Calculate new X   	-> $t2
lw $t8, 0($a2)      	# Store (0,0) matrix value
mul $t2, $t8, $t0   	# Multiply matrix value and old X
lw $t8, 4($a2)      	# Store (0,1) matrix value
mul $t7, $t8, $t1   	# Multiply matrix value and old Y
add $t2, $t2, $t7   	# Add part1 + part2
lw $t8, 8($a2)      	# Store (0,2) matrix value
add $t2, $t2, $t8   	# Add to get new X

# Calculate new Y  	 -> $t3
lw $t8, 12($a2)   	 # Store (1,0) matrix value
mul $t3, $t8, $t0 	 # Multiply matrix value and old X
lw $t8, 16($a2)   	 # Store (1,1) matrix value
mul $t7, $t8, $t1  	 # Multiply matrix value and old Y
add $t3, $t3, $t7  	 # Add part1 + part2
lw $t8, 20($a2)    	 # Store (1,2) matrix value
add $t3, $t3, $t8  	 # Add to get new Y

# Get pixel index of ($t2, $t3)   -> $t9
mul $t8, $t3, $a3  	 # Part1 to calcualte offset -> row_idx * num_of_columns 
add $t8, $t2, $t8  	 # Part2 to calcualte offset -> (row_idx * num_of_columns) + col_idx
add $t9, $a0, $t8  	 # element_addr = base_addr + element_size_in_bytes * offset -> $a0 + offset

# Copy and paste pixel only if it is a valid pixel within buffer
blt $t3, $zero, invalid
bge $t3, $a3, invalid
blt $t2, $zero, invalid
bge $t2, $a3, invalid
lbu $t7, 0($t9)    	 # Store pixel in $t7
sb $t7, 0($a1)    	 # Store pixel in output buffer
j prep
invalid:
sb $zero, 0($a1)
...
prep:
addi $a1, $a1, 1  	  # Write to next byte
